...
* **Code Editor (VS Code) workspace**: Your "development environment" is a browser-based VS Code (Code Editor / code-server) that runs on an EC2 instance provisioned by the workshop's CloudFormation template (`code-editor.yaml`). You reach it through the stack's `VSCodeURL` output, and it comes with the Alien Attack repository pre-cloned and ready for development.
